...
All these drawbacks are solved with _link:VacuumAnalyze.adoc[AUTOVACUUM]_ and Postgres comes with an auto_vacuum daemon that will run at a configurable frequency. Also use another technique called _link:HOT.adoc[HOT] (Heap Only Tuple)_ to deal cope with indexes in MVCC.